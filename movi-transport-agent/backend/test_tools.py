...
async def test_read_tools(db: AsyncSession):
    ...
    print("\n3. Testing list_stops_for_path(path_id=1)")
    result = await list_stops_for_path(path_id=1, db=db)
    print_result("List Stops for Path-1", result)

    # Test 4: List routes by path (path_id=2 is "Path-2")
    print("\n4. Testing list_routes_by_path(path_id=2)")
    result = await list_routes_by_path(path_id=2, db=db)
...
async def test_create_tools(db: AsyncSession):
    ...
    # Test 1: Create a new stop
    print("\n1. Testing create_stop()")
    result = await create_stop(
        name="Test Stop - Indiranagar",
        latitude=12.9716,
        longitude=77.6412,
        db=db
    )
    ...
    result = await create_path(
        name="Test Path - Express",
        stop_ids=[1, 2, 3],  # Using existing stops
        db=db
    )
    print_result("Create Path", result)
    created_path_id = result.get("data", {}).get("path_id")
